...
#  require(grid)
  ...
  plotty <- plotty + geom_segment(data=plotloadings,aes(x=V3,y=V3,xend=V1,yend=V2),color=colors$loadings,arrow=arrow(angle=arrowshapes[1],length=unit(arrowshapes[2],"npc")),environment=localenv,na.rm=TRUE)
  plotty <- plotty + geom_text(data=plotloadings,aes(x=V4,y=V5),color=colors$loadings,label=rownames(plotloadings),size=textsize,environment=localenv,na.rm=TRUE)
  plotty <- plotty + theme(panel.background=element_rect(fill=colors$background),plot.title=element_text(size=rel(1.5),face="bold"))
  print(plotty)
}
## Version RPackage04 
## V1 <- V2 <- V3 <- V4 <- V5 <- NULL wegen biplot.sprm: no visible binding for global variable ‘V1’